...
const ProjectCard = ({
  ...
  return (
    <motion.div
      ...
      className="group relative bg-gradient-to-br from-gray-900/90 to-gray-800/60 backdrop-blur-xl border border-gray-700/40 overflow-hidden hover:border-gray-500/60 transition-all duration-700 hover:shadow-2xl hover:shadow-black/50 hover:-translate-y-2 hover:scale-[1.02]"
    >
      {/* Glow effect on hover */}
      <div className="absolute inset-0 bg-gradient-to-br from-gray-600/5 to-gray-400/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
      ...
      <div className="absolute top-4 left-4 right-4 z-20 flex justify-between items-start">
        ...
        <div className="flex">
          {showFeaturedBadge && project.featured && (
            <div className="px-3 py-1.5 bg-gradient-to-r from-black/70 to-gray-900/70 backdrop-blur-xl border border-yellow-500/30 text-yellow-100 text-xs font-semibold flex items-center gap-1.5 shadow-xl shadow-black/40">
        ...
        <div className="flex">
          {project.isHackathonProject && (
            <div className="px-3 py-1.5 bg-gradient-to-r from-black/70 to-gray-900/70 backdrop-blur-xl border border-orange-500/30 text-orange-100 text-xs font-semibold flex items-center gap-1.5 shadow-xl shadow-black/40">
        ...
        <div className={`${isCompact ? 'p-5' : 'p-6'} flex flex-col space-y-4`}>
          ...
          <div className="space-y-3">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center text-xs text-gray-400 bg-gray-800/60 px-2 py-1 border border-gray-600/30 backdrop-blur-sm flex-shrink-0">
          ...
          <div className="flex flex-wrap gap-2">
            {project.tags.slice(0, isCompact ? 4 : 5).map((tech, idx) => (
              <span
                key={tech}
                className={`${isCompact ? 'px-2.5 py-1 text-xs' : 'px-3 py-1.5 text-xs'} bg-gray-800/70 text-gray-200 font-medium border border-gray-600/40 hover:bg-gray-700/80 hover:border-gray-500/60 transition-all duration-300 backdrop-blur-sm`}
            ...
            {project.tags.length > (isCompact ? 4 : 5) && (
              <span className={`${isCompact ? 'px-2.5 py-1 text-xs' : 'px-3 py-1.5 text-xs'} bg-gray-800/50 text-gray-400 font-medium border border-gray-600/30`}>
                +{project.tags.length - (isCompact ? 4 : 5)}
              </span>
            )}
          </div>
          ...
          <div className="flex gap-3">
            ...
            {project.githubLink && (
              <motion.a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`${isCompact ? 'py-2.5 px-4' : 'py-3 px-5'} bg-gray-800/70 hover:bg-gray-700/90 text-gray-300 hover:text-white border border-gray-600/40 hover:border-gray-500/60 transition-all duration-300 flex items-center justify-center backdrop-blur-sm hover:shadow-lg hover:shadow-gray-800/30`}
